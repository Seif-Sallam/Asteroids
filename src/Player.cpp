	...
	: m_entitiesVec(entitiesVector)
{
	...
	m_shipTexture.loadFromFile("textures/spaceship.png");
	m_shipTexture.setSmooth(true);
	m_bulletTexture.loadFromFile("textures/fire_blue.png");
	...
	m_bulletSoundBuf.loadFromFile("SoundEffects/laserShot.wav");
}
...
void Player::Update(const float& deltaTime, const sf::Vector2i& screenSize)
{
	if (alive) {
		...
		auto toRadian = [](float degree) { return degree * 3.14 / 180.f; };
		...
		if (position.x > screenSize.x) m_body.setPosition(0.0f, position.y);
		else if (position.y > screenSize.y) m_body.setPosition(position.x, 0.0f);
		else if (position.x < 0.f) m_body.setPosition(screenSize.x, position.y);
		else if (position.y < 0.f) m_body.setPosition(position.x, screenSize.y);
