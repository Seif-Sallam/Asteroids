...
Level::Level(GameState *state)
{
	...
	smallAstroidTexture.loadFromFile(RSC_DIR "textures/rock_small.png");
	smallAstroidTexture.setSmooth(true);
	bigAstroidTexture.loadFromFile(RSC_DIR "textures/rock.png");
...
void Level::Update()
{
	...
	if (m_bSpawnAstroids)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			m_entitiesVec.push_back(new Astroid(newAstroids[i].pos, newAstroids[i].type, newAstroids[i].angle, (type == AstroidSize::BIG) ? &bigAstroidTexture : &smallAstroidTexture));
		}
	}
