...
void Player::Update(const float &deltaTime, const sf::Vector2i &screenSize)
{
	if (alive)
	{
		...
		static auto toRadian = [](float degree)
		{ return double(degree * 3.14 / 180.f); };
		...
		m_direction.x = static_cast<float>(cos(toRadian(m_fAngle)));
		m_direction.y = static_cast<float>(sin(toRadian(m_fAngle)));
		...
		if (position.x > screenSize.x)
			...
			m_body.setPosition(float(screenSize.x), position.y);
		else if (position.y < 0.f)
			m_body.setPosition(position.x, float(screenSize.y));
